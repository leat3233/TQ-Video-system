...
jQuery(function () {
                ...
                (new WebUploader.Uploader()).md5File(file, 0, 40 * 1024 * 1024).progress(function (percentage) {
                    ...
                    $.ajax({
                        ...
                        if (data.ifExist) { //若存在，这返回失败给WebUploader，表明该文件不需要上传
                            ...
                            console.log("before");
                            console.log("后缀名是>>"+data.extname);
                            if(data.extname=='.PNG' || data.extname=='.JPG' || data.extname=='.JPEG'|| data.extname=='.BMP' || data.extname=='.png' || data.extname=='.jpg' || data.extname=='.jpeg'|| data.extname=='.bmp'){
                            $(".coverurl").val(data.url);
                        }else if(data.title){
                                
                                $('.vtime').val(data.duration);
    ...
    uploader = WebUploader.create({
        ...
        accept: {
            title: 'Files',
            extensions: 'jpg,jpeg,png,bmp,rmvb,flv,mp4,mov,3gp,wmv,mkv,mpg,avi,mpeg,avi',
            mimeTypes: '.jpg,.jpeg,.png,.bmp,.rmvb,.flv,.mp4,.mov,.3gp,.wmv,.mkv,.mpg,.avi,.mpeg,.avi'
        },
        duplicate: false,
        chunked: true,
        chunkSize: chunkSize,
    ...
    uploader.on('fileQueued',
        function (file) {
            ...
            html += "                <li>上传速度:<b><font  class=\"uploaded_speed\">0</font></b>KB/秒</li>";
            html += "                <li><span class=\"time_name\">剩余时间</span>:<b><span  class=\"time_left\">00:00</span></b></li>";
            html += "                <li class='last'><span class=\"percent_str\">总进度:<b><span  class=\"percent_num\">0%</span></b></span></li>";
            html += "            </ul></div>";
            html += "        </div>";
            html += "    </div>";
            html += "</div>";
